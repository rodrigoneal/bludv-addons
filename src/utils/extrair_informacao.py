...
def é_dublado(text: str) -> bool:
    padrao = re.compile(r"\b(DUAL|DUBLADO|PT-BR|PORTUGUESE|NACIONAL)\b", re.IGNORECASE)
...
def pegar_links_torrent(soup: BeautifulSoup) -> list[str]:
    padrao = re.compile(r"^magnet:\?xt=urn")
    elementos = [
        elemento_a.get("href") for elemento_a in soup.find_all("a", href=padrao)
    ]
...
def categoria_video(soup: BeautifulSoup):
    categorias = ("Documentário", "Filmes", "Novela", "Anime", "Séries")
...
def data_postagem(soup: BeautifulSoup) -> datetime:
    ...
    if match:
        ...
        return datetime.strptime(atualizado, "%d/%m/%Y")
    return
