...
class AbrirPagina(Element):
    ...
    def esta_aberta(self, url: str) -> bool:
        ...
        try:
            self.find_element(
                self.nav_menu, condition=EC.visibility_of_element_located, time=2
            )
            return True
...
class PegarDados(Element):
    ...
    qualidade_audio = (
        By.XPATH,
        "//em[contains(text(),'Qualidade de Áudio')]/ancestor::span",
    )
    qualidade_video = (
        By.XPATH,
        "//em[contains(text(),'Qualidade de Áudio')]/ancestor::span",
    ...
    def informacoes_filme(self) -> Filme:
        ...
        qualidade_audio = self.find_element(self.qualidade_audio, time=2).text
        qualidade_video = self.find_element(self.qualidade_video, time=2).text
        imdb_id = self.find_element(self.imdb, time=2).get_attribute("href")
        imdbRating = self.find_element(self.imdb, time=2).text
        lancamento = self.find_element(self.lancamento, time=2).text
        poster = self.find_element(self.poster, time=2).get_attribute("src")
        ...
        try:
            torrents_link = [
                torrent
                for torrent in self.execute_script(f"return arrDBLinks")
        ...
        atualizado = limpar_dados.limpar_atualizacao(
            atualizacao.get_attribute("innerHTML")
        )
        sub_index = 0
        for index, torrent in enumerate(torrents):
            resolucao_filme = limpar_dados.limpar_qualidade_torrent(torrent.text)
            _linguagem = self.driver.find_element(
                locate_with(*self.versao_torrent).above(torrent)
            )
